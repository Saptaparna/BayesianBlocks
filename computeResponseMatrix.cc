...
int computeResponseMatrix(std::string infile, std::string outfile){
  ...
  for(int k = 0; k<= h_response->GetNbinsX(); k++) //reco information
  { 
    double sumY = 0;
    for(int l = 0; l<= h_response->GetNbinsY(); l++)//gen information
    {
      sumY += h_response->GetBinContent(k, l);
    }
    
    for(int l = 0; l<= h_response->GetNbinsY(); l++)
    {
  ...
  for(int k = 0; k<= h_responseBB->GetNbinsX(); k++) //reco information
  { 
    double sumY = 0;
    for(int l = 0; l<= h_responseBB->GetNbinsY(); l++)//gen information
    {
      sumY += h_responseBB->GetBinContent(k, l);
    }
    
    for(int l = 0; l<= h_responseBB->GetNbinsY(); l++)
    {
      ...
      std::cout << "h_finalResponseBB->GetBinContent(k, l) = " << h_finalResponseBB->GetBinContent(k, l) << std::endl;
    }
  }
