...
void DrawLimitPlot_BB_Mul7()
{
  ...
  for (unsigned int i_mass=0; i_mass<mass.size(); ++i_mass)
  {
    ...
    std::string filename="BayesianBlocks_BH_Mul7_MBH_"+mass_string+"GeV.log";
...
//    getline(file, line);
  ...
  //TLegend *leg=new TLegend(0.45, 0.70, 0.80, 0.85);
  ...
  leg->Draw();
  c_xsec->SetLogy(); 
  c_xsec->SaveAs("c_xsec_BB_Mul7.png");
  c_xsec->SaveAs("c_xsec_BB_Mul7.pdf");
