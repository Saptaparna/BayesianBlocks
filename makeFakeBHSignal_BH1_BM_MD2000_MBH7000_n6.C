...
int makeLimitTemplates(std::string infile, std::string outfile, double kfactor){
  ...
  for (int i=0; i<nEvents; ++i)
   ...
   h_ST_mul8_BB->Draw();
   /*TF1 *fa1 = new TF1("fa1", "gaus", 2000, 10000);
   h_ST_mul8_BB->Fit("fa1", "", "", 2000, 10000);
   fa1->SetParLimits(0, 675.0, 1000.00);
   h_ST_mul8_BB->Fit("fa1", "", "", 2000, 10000);
   fa1->Draw("same");
   fa1->Write("fit_ST_mul8_BB"); 
   */
   Double_t par[9];
   TF1 *fa1 = new TF1("fa1", "gaus", 2500, 6000);
   TF1 *fa2 = new TF1("fa2", "gaus", 6000, 20000);
   ...
   outFileBB=new TFile("SignalBB_ToySignal/BH_Tree_BH1_BM_MD4000_MBH7000_n6.root", "RECREATE");
